...
std::unique_ptr<Figure> StringToFigure::parseCircle(const std::string& radius)
{
    double param;
...
std::unique_ptr<Figure> StringToFigure::parseRectangle(const std::string& sideA, const std::string& sideB)
{
    double paramA, paramB;
    if (utils::tryParseDouble(sideA, paramA) && utils::tryParseDouble(sideB, paramB)) {
        return std::make_unique<Rectangle>(paramA, paramB);
    }
    else throw std::invalid_argument("Invalid string!");
}

std::unique_ptr<Figure> StringToFigure::parseTriangle(const std::string& sideA, const std::string& sideB, const std::string& sideC)
{
...
std::unique_ptr<Figure> StringToFigure::stringToFigure(const std::string& representation)
{
